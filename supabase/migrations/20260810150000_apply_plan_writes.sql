-- ============================================================================
-- Foreman — Phase 5 · Atomic plan apply
--
-- A confirmed voice plan can contain creates, updates, links, assignments and
-- queued messages. SPEC §9 requires the confirm to apply in ONE transaction —
-- half a plan is worse than none. Same design as apply_task_moves (which
-- remains for simple drags): SECURITY INVOKER so RLS applies to every row,
-- plpgsql so it is all-or-nothing, change_log written in the same commit.
--
-- Operations reference tasks created earlier in the same batch via temp ids
-- ("$t0", "$t1", …). The function resolves them as it inserts.
--
-- Each element of p_ops:
--   { "kind":  "insert_contact" | "insert_task" | "update_task" |
--              "insert_dep" | "insert_assignment" | "insert_message",
--     "temp_id": "$t0",                -- insert_task only, optional
--     "data":  { column: value, … },   -- columns for that table
--     "log":   { "entity_type": …, "action": …, "before": …, "after": … } }
-- ============================================================================
...
returns jsonb  -- { "applied": n, "task_ids": { "$t0": uuid, … } }
language plpgsql
set search_path = ''
as $$
declare
...
comment on function public.apply_plan_writes(uuid, jsonb, public.change_source, text) is
  'Applies a confirmed plan (creates/updates/links/assignments/queued messages) in one transaction with its change_log rows. SECURITY INVOKER: RLS applies.';
